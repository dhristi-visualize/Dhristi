...
window.Components.ArrayVisualization = ({ arr, name }) => {
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-gray-500">indices:</span>
        {arr.slice(0, 20).map((_, idx) => (
          <div key={idx} className="w-12 text-center text-xs text-gray-400">{idx}</div>
        ))}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {arr.length > 20 && <div className="text-gray-400 text-sm">... +{arr.length - 20} more</div>}
      </div>
    </div>
  );
};